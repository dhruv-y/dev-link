...
// @desc        Allow user to create new post
// @access      Private
...
router.put('/like/:id', auth, async (req, res) => {
    try {
        ...
        // push to likes array in Post model
        post.likes.unshift({
            user: req.user.id
        });
...
router.post('/comment/:id', [auth, [check('text', 'Text is required!').not().isEmpty()]]
    , async (req, res) => {
        ...
        try {
            ...
            // push to comments array in POST and response
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
    try {
        ...
        if (!comment) {
            return res.status(400).json({ msg: "Comment missing!" });
        }
        // check user auth to delete comment
